...
const Dash: React.FC = () => {
  ...
  const addToTopRight = () => {
    ...
    if (layoutNode) {
      try {
        const path = getPathToCorner(layoutNode, Corner.TOP_RIGHT) as MosaicBranch[];
        const parent = getNodeAtPath(layoutNode, dropRight(path)) as MosaicParent<string> | null;
        const destination = getNodeAtPath(layoutNode, path) as MosaicNode<string> | null;
  ...
  const handleExpand = (targetId: string) => {
    setCurrentNode((prevNode) => {
      if (!prevNode) return prevNode;
        const expandTargetNode = (node: MosaicNode<string>): MosaicNode<string> => {
        if (typeof node === 'object' && (node.first === targetId || node.second === targetId)) {
          return {
            ...node,
  ...
  const removeNode = (node: MosaicNode<string> | null, targetId: string): MosaicNode<string> | null => {
    if (!node) return null;
  ...
  <div className="flex items-center font-bold text-white mb-2 sm:mb-0">
    react-mosaic <span className="text-gray-400 ml-2 text-xs md:text-sm">v6.1.0</span>
  </div>
  <div className="flex flex-row items-center gap-2">
    ...
    >
      <GoArrowUpRight title="Add Window to Top Right" className="w-4 h-4 md:w-5 md:h-5" />
      ...
      <div className="h-full pb-14">
        <Mosaic<string>
          renderTile={(id, path) => {
            ...
            const company = companies.find((comp) => comp.ticker === selectedTicker) || companies[0];
            return (
              <MosaicWindow<string>
                path={path}
